...
define('SOLA_DONATION_VERSION', '1.4.0');
...
function sola_donation_process_form() {
    ...
    if ($result['success']) {
        ...
        // Include debug info if WP_DEBUG is on
        $error_response = array('message' => $result['message']);
        if (defined('WP_DEBUG') && WP_DEBUG && isset($result['debug'])) {
